...
export function AddUserForm({ setResult = undefined }: AddUserFormProps) {
  ...
  const [role, setRole] = useState<string | undefined>("regular");
  ...
  useEffect(() => {
    ...
    if (
      ...
      setBeltCode("");
    if (["belt"].includes(role ?? "regular")) setBeltCode(belts[0]);
    if (["superAdmin", "admin", "coordinator"].includes(role ?? "regular"))
      setBeltCode("no-belt-code");
  }, [role]);
        ...
        <div className="space-y-1">
          ...
          <Select name="role" required value={role} onValueChange={setRole}>
            <SelectTrigger
              ...
              id="role"
            >
              <SelectValue placeholder="Select a role" aria-label="role" />
            </SelectTrigger>
            <SelectContent>
